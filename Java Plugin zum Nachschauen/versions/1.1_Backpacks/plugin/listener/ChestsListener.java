...
public class ChestsListener implements Listener {
    ...
    public void onInteract(PlayerInteractEvent e) {
        ...
        @EventHandler
        public void onInteractWithDrop(PlayerInteractEvent e){
            Player p = (Player)e.getPlayer();
            ...
            if(e.getAction() == Action.RIGHT_CLICK_BLOCK){
                if(e.getClickedBlock().getType() == Material.BEACON){
                    ...
                    for(Player player : Bukkit.getOnlinePlayers()){
                       player.playSound(player.getLocation(), Sound.ENTITY_ENDERDRAGON_SHOOT, 1, 0);
                       player.playSound(player.getLocation(), Sound.ENTITY_CAT_AMBIENT, 2, 1);
                        ...
                        Inventory inv = Bukkit.createInventory(null, InventoryType.CHEST, "§2Airdrop");
                        List<ItemStack> items = new ArrayList<>();

                        items.add(new ItemStack(Material.DIAMOND_SWORD, 1));
                        items.add(new ItemStack((new Potion(PotionType.STRENGTH, 2, true).toItemStack(1))));
